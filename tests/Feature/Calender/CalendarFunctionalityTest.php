...
class CalendarFunctionalityTest extends TestCase
{
    public function test_holidays_can_obtain()
    {
        $response = $this->getJson(route('calendar.retrieveRegionHolidays'));
    ...
    public function test_holidays_can_obtain_using_group_by_filter()
    {
        ...
        $response = $this->getJson(route('calendar.retrieveRegionHolidays').$params);
